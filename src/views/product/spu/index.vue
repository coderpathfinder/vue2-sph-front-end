<template>
  <div>
    <el-card style="margin: 20px 0px;">
      <CategorySelect @getSelectCategoryId="getSelectCategoryId" :disab="scene != 0" />
    </el-card>
    ...
    <el-card style="margin: 20px 0px;">
      <div v-show="scene == 0">
        ...
        <el-pagination style="margin-top: 20px; text-align: center;" :current-page="page" :page-sizes="[5, 10, 20]"
          :page-size="pageSize" :pager-count="7" layout="prev, pager, next, jumper, ->, sizes,total"
          @current-change="getDataPage" @size-change="sizeChange" :total="total"></el-pagination>
      </div>
...
export default {
  ...
  methods: {
    ...
    async getData() {
      let result = await this.$productApi.spu.reqSpuList(this.page, this.pageSize, this.c3Id)
      if (result.code == 200) {
    ...
    async deleteSpu(row) {
      let result = await this.$productApi.spu.reqRemoveSpu(row.id)
      if (result.code == 200) {
    ...
    async handler(row) {
      ...
      let result = await this.$productApi.sku.reqSkuList(row.id)
      if(result.code == 200) {
        this.loading = false
