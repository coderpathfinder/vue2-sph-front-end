...
export default {
  ...
  methods: {
    ...
    */
    async getRoles() {
      const result = await this.$aclApi.user.getRoles(this.user.id)
    ...
    */
    async assignRole() {
      ...
      this.loading = true
      const result = await this.$aclApi.user.assignRoles(userId, roleIds)
      this.loading = false
    ...
    */
    revomveUsers() {
      this.$confirm('确定删除吗?').then(async() => {
        await this.$aclApi.user.removeUsers(this.selectedIds)
    ...
    */
    async removeUser(id) {
      await this.$aclApi.user.removeById(id)
      this.$message.success('删除成功')
    ...
    */
    async getUsers(page = 1) {
      ...
      const result = await this.$aclApi.user.getPageList(page, limit, searchObj)
