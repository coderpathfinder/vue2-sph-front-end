...
export default {
  ...
  methods: {
    ...
    */
    updateRole(role) {
      this.$aclApi.role.updateById({ id: role.id, roleName: role.roleName })
        .then(result => {
    ...
    */
    addRole() {
      ...
      this.$prompt('请输入新名称', '添加角色', {
        ...
        this.$aclApi.role.save({ roleName: value }).then(result => {
    ...
    */
    getRoles(page = 1) {
      ...
      this.$aclApi.role.getPageList(page, limit, searchObj).then(
        result => {
    ...
    */
    removeRole({ id, roleName }) {
      this.$confirm(`确定删除 '${roleName}' 吗?`, '提示', {
        ...
        const result = await this.$aclApi.role.removeById(id)
    ...
    */
    removeRoles() {
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        ...
        const result = await this.$aclApi.role.removeRoles(ids)
        this.getRoles()
        this.$message({
          type: 'success',
          message: '批量删除成功!'
