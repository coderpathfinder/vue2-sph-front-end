  ...
  <div>
    <el-form ref="form" label-width="80px" :model="spu">
      ...
      <el-form-item label="SPU描述">
        <el-input v-model="spu.description" placeholder="请输入SPU描述" type="textarea" rows="4" />
      </el-form-item>
...
export default {
  ...
  methods: {
    async initSpuData(spu) {
      // 获取SPU信息的数据
      const spuResult = await this.$productApi.spu.reqSpuInfoById(spu.id)
      if (spuResult.code == 200) {
        // 在修改spu的时候,需要向服务器发请求的，把服务器返回的数据（对象），赋值给spu属性
        this.spu = spuResult.data
      }
      // 获取品牌的信息
      const tradeMarkResult = await this.$productApi.spu.reqTradeMarkList()
      if (tradeMarkResult.code == 200) {
        this.tradeMarkList = tradeMarkResult.data
      }
      // 获取spu图片的数据
      const spuImageResult = await this.$productApi.spu.reqSpuImgListById(spu.id)
      ...
      // 获取平台全部的销售属性
      const saleResult = await this.$productApi.spu.reqSaleList()
    ...
    async addOrUpdateSpu() {
      ...
      const result = await this.$productApi.spu.reqAddOrUpdateSpu(this.spu)
      if (result.code == 200) {
        // 提示
