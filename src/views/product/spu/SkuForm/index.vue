...
export default {
  ...
  methods: {
    async initData(c1Id, c2Id, c3Id, spuInfo) {
      ...
      //获取图片
      let result1 = await this.$productApi.sku.reqSpuImageList(spuInfo.id)
      if(result1.code == 200) {
      ...
      //获取销售属性
      let result2 = await this.$productApi.sku.reqSaleAttrList(spuInfo.id)
      if(result2.code == 200) {
        this.spuSaleAttrList = result2.data
      }
      //获取平台属性列表
      let result3 = await this.$productApi.sku.reqAttrInfoList(c1Id, c2Id, c3Id)
      if(result3.code == 200) {
    ...
    async save() {
      ...
      //发送请求
      let result = await this.$productApi.sku.reqAddSku(skuInfo)
      if(result.code == 200) {
        //提示
