<template>
  <div>
    <el-card style="margin: 20px 0px;">
      <CategorySelect :disab="dialogFormVisible" @getSelectCategoryId="getSelectCategoryId" />
    </el-card>
        ...
        <el-table :data="attrList" style="width: 100%;" border>
          ...
          <el-table-column prop="attrValueList" label="属性名称" width="width">
            <template slot-scope="{row}">
              <el-tag v-for="item in row.attrValueList" :key="item.id" type="success" style="margin: 0 5px;">{{
                item.valueName }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column label="操作" width="150">
            <template slot-scope="{row}">
              <el-button type="warning" icon="el-icon-edit" size="mini" @click="showUpdateAttrForm(row)" />
...
export default {
  ...
  computed: {
    canAddAttr() {
      return !((this.c1Id > 0 && this.c2Id > 0 && this.c3Id > 0))
    }
  },
  methods: {
    ...
    async getData() {
      const result = await this.$productApi.attr.reqAttrList(this.c1Id, this.c2Id, this.c3Id)
      if (result.code == 200) {
    ...
    async addOrUpdateAttr() {
      const data = cloneDeep(this.attrInfo)
